...
def score_nutrients(nutrients):
    """Calculates baseline score using simplified/normalized nutrients."""
    ...
    if sugar > 10:
        score -= 2
        breakdown.append({"text": "High sugar content (>10g)", "points": -2})
    if fat > 20:
        score -= 1
        breakdown.append({"text": "High fat content (>20g)", "points": -1})
    if salt > 1.5:
        score -= 2
        breakdown.append({"text": "High salt content (>1.5g)", "points": -2})

    return max(score, 1), breakdown
...
def apply_health_rules(nutrients, ingredients_lower):
    """Applies condition-specific penalties using normalized nutrients."""
    warnings = []
    score_penalty = 0
    breakdown = []
    ...
    for condition in USER_PROFILE["conditions"]:
        ...
        if condition in HEALTH_RULES:
            ...
            for nutrient, limits in rules.get("nutrients", {}).items():
                # rules might use 'sugars_100g' or just 'sugars'
                val = nutrients.get(nutrient)
                if val is None:
                    val = nutrients.get(nutrient.replace('_100g', '') + '_100g')
                if val is None:
                    val = nutrients.get(nutrient.replace('s_100g', '') + '_100g')
                
                nutrient_val = float(val or 0)

                if nutrient_val > limits["max"]:
                    penal = limits["penalty"]
                    score_penalty += penal
                    msg = f"{condition.capitalize()}: High {nutrient.split('_')[0]} ({nutrient_val} > {limits['max']})"
                    breakdown.append({"text": msg, "points": -penal})
                    warnings.extend(rules.get("warnings", []))

            # Ingredient-based penalties
            for bad in rules.get("ingredients", []):
                if bad.lower() in ingredients_lower:
                    penal = rules.get("penalty_points", 3)
                    score_penalty += penal
                    breakdown.append({"text": f"{condition.capitalize()}: Avoid \"{bad}\"", "points": -penal})
                    warnings.extend(rules.get("warnings", []))

    return score_penalty, list(set(warnings)), breakdown
...
def analyze_product_engine(product, ingredients_lower, nutrients_raw):
    """
    Runs the custom scoring logic on the fetched/parsed data.
    """
    # 1. Simplify nutrients FIRST for consistent calculation
    nutrients = simplify_nutrients(nutrients_raw)
    ...
    # 2. Score baseline
    base_score, baseline_breakdown = score_nutrients(nutrients)

    # 3. Apply health penalties
    health_penalty, health_warnings, health_breakdown = apply_health_rules(nutrients, ingredients_lower)

    # 4. Apply allergy penalties
    allergy_penalty = 0
    allergy_warnings = []
    allergy_breakdown = []
    for allergen in USER_PROFILE["allergies"]:
        if allergen.lower() in ingredients_lower:
            p = 5
            allergy_penalty += p
            allergy_breakdown.append({"text": f"ALLERGY: Contains {allergen}", "points": -p})
            allergy_warnings.append(f"[HIGH] Contains allergen: {allergen}")

    # Combine all penalties
    total_penalty = health_penalty + allergy_penalty
    final_score = max(base_score - total_penalty, 1)

    # Consolidate breakdown
    full_breakdown = baseline_breakdown + health_breakdown + allergy_breakdown
    ...
    all_warnings = list(set(
        base_warnings + health_warnings + allergy_warnings + extra_warnings
    ))
    ...
    return {
        ...
        "score10": float(final_score),
        "score100": float(final_score * 10),
        "tier": final_tier,
        "confidence": confidence,
        "personalizedWarnings": all_warnings,
        "breakdown": full_breakdown,
        "explanation": f"Score: {int(final_score)}/10 — {final_tier}"
    }
